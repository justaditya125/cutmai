...
def log_suspicious_activity(ip_or_user: str, activity_type: str, description: str, risk_level: str = "MEDIUM"):
    """Logs a security event in memory and persists it into the MongoDB security_logs collection"""
    ...
    # Persist to MongoDB security_logs collection
    try:
        db = get_db()
        if db is not None:
        ...
        print(f"[Logger] Failed to persist security log to MongoDB: {e}")
