...
class DatabaseManager:
    ...
    def create_user(self, email, password=None, name=None, login_method='email', google_id=None, profile_picture=None, is_admin=False, is_approved=None):
...
def setup_database():
    ...
    if db_mgr.connect():
        ...
        if db_mgr.create_collections_and_indexes():
            ...
            res_approve = db_mgr.db.users.update_many(
                {"is_approved": {"$exists": False}},
                {"$set": {"is_approved": True}}
            )
            if res_approve.modified_count > 0:
                print(f"✅ Backfilled is_approved: True on {res_approve.modified_count} users")

            # Backfill token_limit: 1000000 on existing users that do not have this field
            res_limit = db_mgr.db.users.update_many(
                {"token_limit": {"$exists": False}},
                {"$set": {"token_limit": 1000000}}
            )
            if res_limit.modified_count > 0:
                print(f"✅ Backfilled token_limit: 1000000 on {res_limit.modified_count} users")
